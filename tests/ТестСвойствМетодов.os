...
Перем Ф, КолОшибок, ИмяВременногоФайла, Таймер, ПолеВвода1, Форма1, Сч1, Сч2, Сч3, Сч4, Сч5, Сч6;
Перем ТекстТеста, СтрКода0, СтрСозданияОбъекта, СтрСозданияОбъекта2, СписокИменПараметров, ФормаNotifyIcon;
Перем СписокОшибокТестовыхКодов, СписокОшибокДляСвойств, СписокОшибокМетодов, КаталогСправки;
		...
		Если М.Количество() > 0 Тогда
			Для А2 = 0 По М.Количество() - 1 Цикл
				...
				Если Не (СокрЛП(ТестовыйКод) = "") Тогда
					...
					ТекстДокХХХ.Записать(ИмяВременногоФайла);
					Команда1("""C:\Program Files\OneScript\bin\oscript.exe""", ИмяВременногоФайла, ПолеВвода1);
					Форма1.Фокус();
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция Команда1(ИмяФайла, Аргументы, Объект)
	...
	Если Не Процесс1.Завершен Тогда
		...
		Пока Стр1 <> Неопределено Цикл
			...
			Если (СтрНайти(Стр3, "!!!") > 0) или (СтрНайти(Стр3, "{") > 0)  или (Двоеточие <> ":") Тогда
				...
				СписокОшибокТестовыхКодов.Добавить(Стр3);
			КонецЕсли;
	...
	Если Не Процесс1.Завершен Тогда
		...
		Пока Стр1 <> Неопределено Цикл
			Если Стр1 <> "Тест завершен." Тогда
				...
				ПервыйСимвол = Лев(Стр1, 1);
				Если ПервыйСимвол = "{" Тогда
					СписокОшибокДляСвойств.Добавить(Стр1);
				ИначеЕсли Не (ДваПоследнихСимвола = ".)") и (  (Не (ДваПоследнихСимвола = "=="))  и  (Не (ДваПоследнихСимвола = "--"))  ) Тогда
					СписокОшибокДляСвойств.Добавить("" + КлассОшибки + " - свойство - " + Стр1);
				КонецЕсли;
				Объект.ДобавитьТекст("" + Стр1 + Символы.ПС);
			КонецЕсли;
			Стр1 = Процесс1.СтандартныйВывод.ПрочитатьСтроку();
...
Функция ПолучитьОбъектДляМетодов(КлассРус, КлассАнгл)
	...
	ИначеЕсли (КлассАнгл = "MenuNotifyIcon") Тогда // МенюЗначкаУведомления (MenuNotifyIcon)
		...
		|МенюЗначкаУведомления1 = Ф.МенюЗначкаУведомления();
		|МенюЗначкаУведомления1.ПриПоявлении = ""МенюЗначкаУведомления1_ПриПоявлении()"";
		|ЭлементМеню1 = МенюЗначкаУведомления1.ЭлементыМеню.Добавить(Ф.ЭлементМеню(""1 меню""));
		|ЭлементМеню1.Отображать = Истина;
		|ЭлементМеню2 = МенюЗначкаУведомления1.ЭлементыМеню.Добавить(Ф.ЭлементМеню(""2 меню""));
		|ЭлементМеню2.Отображать = Истина;
		|ЗначокУведомления1 = Ф.ЗначокУведомления(МенюЗначкаУведомления1);
		|
		|НекорректныеСвойства = Новый СписокЗначений;
		|
		|Попытка
		|	ЭлементМеню1.Текст = МенюЗначкаУведомления1.ЭлементМеню(0).Текст;
		|
		|Исключение НекорректныеСвойства.Добавить(""ЭлементыМеню (MenuItems)""); КонецПопытки;
		|
		|Если НекорректныеСвойства.Количество() > 0 Тогда
...
Сообщить("Начат тест тестовых кодов");
ПолеВвода1.ДобавитьТекст("Начат тест тестовых кодов" + Символы.ПС);
КолОшибокТестовыхКодов = 0;
СписокОшибокТестовыхКодов = Новый СписокЗначений;
ТестированиеКодов();
Сообщить("Ошибок ТестовыхКодов = " + СписокОшибокТестовыхКодов.Количество());
ПолеВвода1.ДобавитьТекст("Ошибок ТестовыхКодов = " + СписокОшибокТестовыхКодов.Количество() + Символы.ПС);
Сообщить("Тест тестовых кодов выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата());
ПолеВвода1.ДобавитьТекст("Тест тестовых кодов выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата() + Символы.ПС);

Сообщить("==============================================================================================");
ПолеВвода1.ДобавитьТекст("==============================================================================================" + Символы.ПС);
Сообщить("Начат тест методов");
ПолеВвода1.ДобавитьТекст("Начат тест методов" + Символы.ПС);
СписокОшибокМетодов = Новый СписокЗначений();
ТестированиеМетодов();
ФормаNotifyIcon.Закрыть();
Сообщить("Ошибок методов = " + СписокОшибокМетодов.Количество());
ПолеВвода1.ДобавитьТекст("Ошибок методов = " + СписокОшибокМетодов.Количество() + Символы.ПС);
Сообщить("Тест методов выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата());
ПолеВвода1.ДобавитьТекст("Тест методов выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата() + Символы.ПС);

Сообщить("==============================================================================================");
ПолеВвода1.ДобавитьТекст("==============================================================================================" + Символы.ПС);
Сообщить("Начат тест свойств");
ПолеВвода1.ДобавитьТекст("Начат тест свойств" + Символы.ПС);
СписокОшибокДляСвойств = Новый СписокЗначений();
ТестированиеСвойств();
ФормаNotifyIcon.Закрыть();
Сообщить("Ошибок свойств = " + СписокОшибокДляСвойств.Количество());
ПолеВвода1.ДобавитьТекст("Ошибок свойств = " + СписокОшибокДляСвойств.Количество() + Символы.ПС);
Сообщить("Тест свойств выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата());
ПолеВвода1.ДобавитьТекст("Тест свойств выполнен за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата() + Символы.ПС);

Сообщить("==============================================================================================");
ПолеВвода1.ДобавитьТекст("==============================================================================================" + Символы.ПС);
Сообщить("Всего ошибок = " + СписокОшибокДляСвойств.Количество());
ПолеВвода1.ДобавитьТекст("Всего ошибок = " + (СписокОшибокТестовыхКодов.Количество() + СписокОшибокМетодов.Количество() + СписокОшибокДляСвойств.Количество()) + Символы.ПС);
Для А = 0 По СписокОшибокТестовыхКодов.Количество() - 1 Цикл
	Сообщить("" + СписокОшибокТестовыхКодов.Получить(А).Значение);
	ПолеВвода1.ДобавитьТекст("" + СписокОшибокТестовыхКодов.Получить(А).Значение + Символы.ПС);
КонецЦикла;
Для А = 0 По СписокОшибокМетодов.Количество() - 1 Цикл
	Сообщить("" + СписокОшибокМетодов.Получить(А).Значение);
	ПолеВвода1.ДобавитьТекст("" + СписокОшибокМетодов.Получить(А).Значение + Символы.ПС);
КонецЦикла;
Для А = 0 По СписокОшибокДляСвойств.Количество() - 1 Цикл
	Сообщить("" + СписокОшибокДляСвойств.Получить(А).Значение);
	ПолеВвода1.ДобавитьТекст("" + СписокОшибокДляСвойств.Получить(А).Значение + Символы.ПС);
КонецЦикла;
