// BSLLS-off
// Перед тестированием измените раскладку на русскую для успешного тестирование отправки кодов клавиш.
...
Перем СписокОшибок, КаталогСправки, КаталогБиблиотеки, КаталогТестов, М_Свойств;
...
Перем ПропуститьИнтерактивныеНаследуемыеМетоды;
...
Процедура ТестированиеМетодов()
	...
	Для А1 = 0 По Массив3.ВГраница() Цикл
		...
		Для А = 0 По Массив4.ВГраница() Цикл
			...
			Массив3.ВГраница() + ") " + КлассРус + " (" + КлассАнгл + ")");
			...
			Если ПропуститьИнтерактивныеНаследуемыеМетоды.НайтиПоЗначению(КлассАнгл) = Неопределено Тогда
				...
				// // // ИмяВременногоФайла = "C:\Users\master\AppData\Local\Temp\" + КлассРус + "-методы.tmp";
				// // // ТекстДокХХХ.Записать(ИмяВременногоФайла, "UTF-8");
...
Процедура ТестированиеСвойств();
	...
	Для А1 = 0 По Массив3.ВГраница() Цикл
		...
		Для А = 0 По Массив4.ВГраница() Цикл
			...
			// // // ИмяВременногоФайла = "C:\Users\master\AppData\Local\Temp\" + КлассРус + "-свойства.tmp";
			// // // ТекстДокХХХ.Записать(ИмяВременногоФайла, "UTF-8");
...
Процедура ТестированиеКодов()
	...
	Для А = 0 По ВыбранныеФайлы.ВГраница() Цикл
		...
		Если ПропуститьСвойствоИлиМетодПриТестированииКодов.НайтиПоЗначению(ВыбранныеФайлы[А].ПолноеИмя) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		...
		Если М.Количество() > 0 Тогда
			Для А2 = 0 По М.Количество() - 1 Цикл
				ТестовыйКод = СтрНайтиМежду(М[А2], "<DIV id=""cont", "</DIV>", Ложь, )[0];
				...
				Если Не (СокрЛП(ТестовыйКод) = "") Тогда
					...
					ИмяВременногоФайла = СтрЗаменить(ИмяВременногоФайла, "OneScriptForms.", "");
					ТекстДокХХХ.Записать(ИмяВременногоФайла, "UTF-8");
					...
					// // // ПодстрокаПоиска = "Новый Действие(ЭтотОбъект,";
					// // // ПодстрокаЗамены = "Ф.Действие(ЭтотОбъект,";
					// // // ТестовыйКод = СтрЗаменить(ТестовыйКод, ПодстрокаПоиска, ПодстрокаЗамены);
					...
					Форма1.Фокус();
					Ф.ПередатьУправление();
					Если (Не Ф.ПлатформаWin) или (СтрНайти(ТестовыйКод, "НажатьКнопкуМыши") > 0) Тогда
						Команда1("oscript", ИмяВременногоФайла, ПолеВвода1, ВыбранныеФайлы[А].Имя, ТестовыйКод);
					Иначе
						Команда1("""C:\Program Files\OneScript\bin\oscript.exe""", ИмяВременногоФайла, ПолеВвода1, ВыбранныеФайлы[А].Имя, ТестовыйКод);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция Команда1(ИмяФайла, Аргументы, Объект, ИмяФайлаСправки, Текст_Теста)
	...
	Если Не Процесс1.Завершен Тогда
		...
		Пока Стр1 <> Неопределено Цикл
			...
			Если (СтрНайти(Стр3, "!!!") > 0) или (СтрНайти(Стр3, "{") > 0)  или (Двоеточие <> ":") или (СтрНайти(Стр3, "Unhandled Exception:") > 0) Тогда
			...
			Объект.НачалоВыделения = Объект.ДлинаТекста - СтрДлина(Стр3 + "  " + ИмяФайлаСправки + Символы.ПС);
...
Попытка
	КаталогСправки = "C:\444";// без слэша в конце
	КаталогБиблиотеки = "C:\444\111\OneScriptForms\OneScriptForms\bin\Debug";// без слэша в конце
	КаталогТестов = "C:\444\Тесты\"; // со слэшем в конце
...
Если Ф.ПлатформаWin Тогда
	УдалитьФайлы("C:\444\Тесты", "*.os");
Иначе
	УдалитьФайлы("/home/vlad/Projects/444/Тесты", "*.os");
КонецЕсли;
...
Если Не Ф.ПлатформаWin Тогда // Исключим объекты, не работающие в линукс.
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Clipboard.GetImage.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Clipboard.SetImage.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Control.BeginUpdate.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Control.EndUpdate.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewButtonCell.FlatStyle.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewCell.Displayed.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewCheckBoxCell.FlatStyle.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewCheckBoxColumn.FlatStyle.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewComboBoxCell.MaxDropDownItems.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewComboBoxCell.Sorted.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.AlwaysGroupOnText.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.CollapseAll.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.DataGridView.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.DataGridViewGrouperStyle.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.ExpandAll.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.ForceAsText.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.GroupName.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.GroupSortOrder.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.GroupSortOrder.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.RemoveGroup.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.SelectRowsOnDoubleClick.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.SetGroup.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.ShowCount.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.ShowGroupName.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.ShowGroupName.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouper.StartCollapsed.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperConstructor.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.Grouper.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.Left.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.Parent.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.PlaceBottom.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.PlaceLeft.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.PlaceRight.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.PlaceTop.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.Top.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControl.Width.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewGrouperControlConstructor.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DataGridViewHeaderCell.Frozen.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.DateTimePicker.Text.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Form.Parent.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Form.PinToDesktop.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Form.UnpinFromDesktop.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItem.GridItemType.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItem.GridItems.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItem.Label.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItem.Value.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItemCollection.Count.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.GridItemCollection.Item.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.Click.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.ContextMenu.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.DoubleClick.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.Icon.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.MouseDown.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.MouseMove.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.MouseUp.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.Text.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIcon.Visible.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.NotifyIconConstructor.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.DataGridViewGrouper.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.DataGridViewGrouperControl.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.DoMessageBox.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.HideConsole.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.MinimizedConsole.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.NotifyIcon.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.RestoreConsole.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.SendKeys.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.GridItems.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.GridItems2.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.PropertyValueChanged.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.SelectedGridItem.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.SelectedGridItemChanged.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGridConstructor.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.PropertyGrid.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.SelectedObject.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.ToolbarVisible.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyGrid.PropertySort.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.FindWindowByCaption.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.OneScriptForms.MouseKeyPress.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.RichTextBox.Rtf.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.RichTextBox.SelectionIndent.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.Form.WindowState.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyValueChangedEventArgs.ChangedItem.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.PropertyValueChangedEventArgs.OldValue.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.SelectedGridItemChangedEventArgs.NewLabel.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.SelectedGridItemChangedEventArgs.NewValue.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.SelectedGridItemChangedEventArgs.OldLabel.html");
	ПропуститьСвойствоИлиМетодПриТестированииКодов.Добавить(КаталогСправки + "/OneScriptFormsru/OneScriptForms.SelectedGridItemChangedEventArgs.OldValue.html");
КонецЕсли;
