Перем Ф, ТекстТеста, СтрКода0, СписокСообщений, СписокМетодовБезПовторов;
Перем СписокКлассовСИменемМетодов, СтрСозданияОбъекта, СтрСозданияОбъекта2, СписокИменПараметров;
Перем Сч1, Сч2, Сч3, Сч4, Сч5, Сч6;
Перем СписокОшибок;
		...
		Пока Стр1 <> Неопределено Цикл
			Если Стр1 <> "Тест завершен." Тогда
				...
				ИначеЕсли Не (ДваПоследнихСимвола = ";;") и (  (Не (ДваПоследнихСимвола = "=="))  и  (Не (ДваПоследнихСимвола = "--"))  ) Тогда
					...
					ТекстДляФайлаОшибок = "" + КлассОшибки + " - метод - " + Стр1;
				КонецЕсли;
				Если ТекстДляФайлаОшибок <> "" Тогда
					ТекстДокХХХ = Новый ТекстовыйДокумент;
					ТекстДокХХХ.Прочитать("C:\444\ОшибкиТестаМетодов.txt");
					ТекстДокХХХ.ДобавитьСтроку(ТекстДляФайлаОшибок);
					ТекстДокХХХ.ДобавитьСтроку("===============================================");
					Приостановить(2000);
					ТекстДокХХХ.Записать("C:\444\ОшибкиТестаМетодов.txt");
				КонецЕсли;
...
Функция ПолучитьОбъект(КлассРус, КлассАнгл)
	...
	Иначе
		...
		|        ТекстДокХХХ.Прочитать(""C:\444\ОшибкиТестаМетодов.txt"");
		|        ТекстДокХХХ.ДобавитьСтроку(Новый Файл(ТекущийСценарий().Источник).Имя);
		|        ТекстДокХХХ.ДобавитьСтроку(""Не создан объект."");
		|        ТекстДокХХХ.ДобавитьСтроку(""==============================================="");
		|        ТекстДокХХХ.Записать(""C:\444\ОшибкиТестаМетодов.txt"");
		|    
        // |        ЗавершитьРаботу(1);
...
Процедура Тестирование()
	...
	Для А1 = 0 По Массив3.ВГраница() Цикл
		...
		Для А = 0 По Массив4.ВГраница() Цикл
			...
			|	ТекстДокХХХ.ДобавитьСтроку(Новый Файл(ТекущийСценарий().Источник).Имя);
			|	Для А = 0 По НекорректныеМетоды.Количество() - 1 Цикл
			|		ТекстДокХХХ.ДобавитьСтроку("""" + Символы.Таб + НекорректныеМетоды.Получить(А).Значение);
			|	КонецЦикла;
			|	ТекстДокХХХ.ДобавитьСтроку(""==============================================="");
			|	ТекстДокХХХ.Записать(""C:\444\ОшибкиТестаМетодов.txt"");
			|КонецЕсли;
	...
	Сообщить("параметров нет " + Сч1);
	Сообщить("параметров нет, возвращаемого значения нет " + Сч2);
	Сообщить("параметров нет, возвращаемое значение есть " + Сч3);
	Сообщить("параметры есть " + Сч4);
	Сообщить("параметры есть, возвращаемого значения нет " + Сч5);
	Сообщить("параметры есть, возвращаемое значение есть " + Сч6);
			...
			Если Параметры = "" Тогда // параметров нет (588 шт)
				...
				Иначе // параметров нет, возвращаемое значение есть (66 шт)=====================================================================
					...
					Если ВозвращаемоеЗначение = "Тип: Строка." Тогда
						// Сообщить("" + СтрКода4 + "===");
						// Сообщить("" + Символы.Таб + ВозвращаемоеЗначение);
						ТипВозврата = "Строка";
			...
			Иначе // параметры есть (358 шт)
				...
				Если ВозвращаемоеЗначение = "" Тогда // параметры есть, возвращаемого значения нет (157 шт)=============================
					...
					Для А6 = 0 По М_Параметр.ВГраница() Цикл
						...
						// Сообщить("ПараметрВСкобки = " + ПараметрВСкобки);
						// Сообщить("ТипПараметра = " + ТипПараметра);
											
						Если ТипПараметра = "Тип: Строка." Тогда //  Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка. Строка.
							СтрУстановкиПараметра = СтрУстановкиПараметра + Символы.Таб + ПараметрВСкобки + " = ""Произвольный текст"";" + Символы.ПС;
						ИначеЕсли ТипПараметра = "Тип: Число." Тогда // Число. Число. Число. Число. Число. Число. Число. Число. Число. Число. Число. Число. Число. Число. Число.
							...
							КонецЕсли;
						ИначеЕсли ТипПараметра = "Тип: Булево." Тогда //  Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево.
							СтрУстановкиПараметра = СтрУстановкиПараметра + Символы.Таб + ПараметрВСкобки + " = Истина;" + Символы.ПС;
						ИначеЕсли СтрНайти(ТипПараметра, "Тип: <A href=") > 0 Тогда // Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект 
						ИначеЕсли ТипПараметра = "Тип: Произвольный." Тогда //  Произвольный. Произвольный. Произвольный. Произвольный. Произвольный. Произвольный. Произвольный.
					...
					КонецЦикла;	
					// Сообщить("СтрУстановкиПараметров = " + Символы.ПС + СтрУстановкиПараметров);
					СтрПараметровВсех = СтрПараметровВсех + ")";
					СтрПараметровОбязательных = СтрПараметровОбязательных + ")";
					...
					// Сообщить("" + КлассРус + "1." + МетодРус + СтрПараметровВсех + "===все параметры====================================");
					// Сообщить("" + КлассРус + "1." + МетодРус + СтрПараметровОбязательных + Символы.ПС + Символы.ПС);
				...
				Иначе // параметры есть, возвращаемое значение есть (201 шт)=====================================================================
					...
					Для А6 = 0 По М_Параметр.ВГраница() Цикл
						...
						ИначеЕсли ТипПараметра = "Тип: Булево." Тогда //  Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево. Булево.
							СтрУстановкиПараметра = СтрУстановкиПараметра + Символы.Таб + ПараметрВСкобки + " = Истина;" + Символы.ПС;
						ИначеЕсли СтрНайти(ТипПараметра, "Тип: <A href=") > 0 Тогда // Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект Объект
						ИначеЕсли СтрНайти(ТипПараметра, ";") > 0 Тогда // параметр может быть разных типов
					...
					Если СтрМетодаСоВсемиПараметрами = СтрМетодаСОбязательнымиПараметрами Тогда
						...
						|";
					Иначе
						// Сообщить("СтрМетодаСоВсемиПараметрами = " + СтрМетодаСоВсемиПараметрами);
						М08.Добавить(СтрМетодаСоВсемиПараметрами);
...
КонецФункции//МетодыТест(ИмяФайлаЧленов, КлассРус, КлассАнгл)
...
Для А = 0 По ВыбранныеФайлы.ВГраница() Цикл
	...
	Если (ВыбранныеФайлы[А].Имя = "ДиалогВыбораКаталога(FolderBrowserDialog).os") или 
		(ВыбранныеФайлы[А].Имя = "ДиалогВыбораЦвета(ColorDialog).os") или 
		(ВыбранныеФайлы[А].Имя = "ДиалогВыбораШрифта(FontDialog).os") или 
		(ВыбранныеФайлы[А].Имя = "ДиалогОткрытияФайла(OpenFileDialog).os") или 
		(ВыбранныеФайлы[А].Имя = "ДиалогСохраненияФайла(SaveFileDialog).os") Тогда // эти объекты интерактивны, тестировать отдельно вручную
	Иначе
		Сообщить(" (" + Ф.Математика().Окр(((ТекущаяУниверсальнаяДатаВМиллисекундах() - Таймер)/1000)/60, 2) + " мин." + " " + КолТестируемыхКлассов + " из " + ВыбранныеФайлы.Количество() + ")" + 
				"""C:\Program Files\OneScript\bin\oscript.exe"" " + ВыбранныеФайлы[А].ПолноеИмя);
		Команда("""C:\Program Files\OneScript\bin\oscript.exe""", ВыбранныеФайлы[А].ПолноеИмя, ПолеВвода1);
	КонецЕсли;
	Форма1.Фокус();
КонецЦикла;
