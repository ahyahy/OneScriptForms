Перем Ф, ТекстТеста, СтрКода0, СтрСозданияОбъекта, СтрСозданияОбъекта2, СписокИменПараметров, ФормаNotifyIcon;
Перем Сч1, Сч2, Сч3, Сч4, Сч5, Сч6, СписокОшибок, ИмяВременногоФайла, Таймер, КолТестируемыхКлассов, ПолеВвода1, Форма1;

Функция Команда(ИмяФайла, Аргументы, Объект, КлассОшибки)
	...
	Для А1 = 0 По Массив3.ВГраница() Цикл
		...
		Для А = 0 По Массив4.ВГраница() Цикл
			...
			Иначе
				...
				Команда("""C:\Program Files\OneScript\bin\oscript.exe""", ИмяВременногоФайла, ПолеВвода1, КлассРус + " (" + КлассАнгл + ")");
			КонецЕсли;
			Форма1.Фокус();
		КонецЦикла;
	КонецЦикла;
...
ПолеВвода1.ДобавитьТекст("Протестировано классов = " + КолТестируемыхКлассов + Символы.ПС);
ПолеВвода1.ДобавитьТекст("Выполнено за: " + ((ТекущаяУниверсальнаяДатаВМиллисекундах()-Таймер)/1000)/60 + " мин." + " " + ТекущаяДата() + Символы.ПС);
